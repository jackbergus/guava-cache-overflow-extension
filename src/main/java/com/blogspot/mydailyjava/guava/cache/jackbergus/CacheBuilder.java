...
public class CacheBuilder {
    ...
    public static <K,V> CacheMap<K,V> createTmpCacheBuilder() {
        try {
            ...
            f.deleteOnExit();
            return new CacheMap<>((FileSystemPersistingCache<K, V>) FileSystemCacheBuilder.<K,V>newBuilder().persistenceDirectory(f).maximumSize(TMP_CACHE_SIZE).softValues().build());
